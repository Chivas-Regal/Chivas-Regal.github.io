...
易得$C_{n} = B * C_{n - 1} = B * B * C_{n - 2} =...= B^{n - 1} * C_{1}$，且$f[n - 1]$也可以由B同行矩阵从上一次的$C[n - 1]$中获取  
...
<table>
<tr>
<td>$f[n]$</td><th rowspan="4">$=$</th><td>$a$</td><td>$0$</td><td>$b$</td><td>$1$</td><th rowspan="4">$\times$</th><td>$f[n-1]$</td><th rowspan="4">=</th><td>$a$</td><td>$0$</td><td>$b$</td><td>$1$</td><td rowspan="4" valign="top">$\^(n-2)$</td><th rowspan="4">*</th><td>$f[2]$</td>
</tr>
<tr>
